...
def _sparse_cosine_top_n_standard(matrix_a: csc_matrix,
    ...
    """
    A method for sparse matrix multiplication followed by an argpartition to
...
def sparse_cosine_top_n(matrix_a: Union[csc_matrix, coo_matrix], 
    ...
    top_n: int
        The best n matches that should be returned
    low_memory: int
        An int inidcating how much memory can be used by the function. If 0 the least amount
        of memory is used, if higher it indicates the number of rows which should be 
    ...
    if low_memory == 0:
        ...
        return _sparse_cosine_top_n_standard(matrix_a, matrix_b, low_memory, top_n, verbose)