...
def test_indel(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(indel=True).popitem()[1][0].sim(string_a, string_b)
        )
        == expected
    )
...
def test_discounted_levenshtein(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(discounted_levenshtein=True)
            .popitem()[1][0]
            .sim(string_a, string_b)
        )
        == expected
    )
...
def test_tichy(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(tichy=True).popitem()[1][0].sim(string_a, string_b)
        )
        == expected
    )
...
def test_cormodeL_z(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(cormodel_z=True)
            .popitem()[1][0]
            .sim(string_a, string_b)
        )
        == expected
...
def test_iterative_sub_string(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(iterative_sub_string=True)
            .popitem()[1][0]
            .sim(string_a, string_b)
        )
        == expected
...
def test_baulieu_xiii(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(baulieu_xiii=True)
            .popitem()[1][0]
            .sim(string_a, string_b)
        )
        == expected
...
def test_clement(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(clement=True).popitem()[1][0].sim(string_a, string_b)
        )
        == expected
    )
...
def test_dice_asymmetricI(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(dice_asymmetrici=True)
            .popitem()[1][0]
            .sim(string_a, string_b)
        )
        == expected
...
def test_kuhns_iii(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(kuhns_iii=True)
            .popitem()[1][0]
            .sim(string_a, string_b)
        )
        == expected
...
def test_overlap(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(overlap=True).popitem()[1][0].sim(string_a, string_b)
        )
        == expected
    )
...
def test_pearson_ii(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(pearson_ii=True)
            .popitem()[1][0]
            .sim(string_a, string_b)
        )
        == expected
...
def test_weighted_jaccard(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(weighted_jaccard=True)
            .popitem()[1][0]
            .sim(string_a, string_b)
        )
        == expected
...
def test_warrens_iv(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(warrens_iv=True)
            .popitem()[1][0]
            .sim(string_a, string_b)
        )
        == expected
...
def test_bag(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(bag=True).popitem()[1][0].sim(string_a, string_b)
        )
        == expected
    )
...
def test_rouge_l(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(rouge_l=True).popitem()[1][0].sim(string_a, string_b)
        )
        == expected
    )
...
def test_ratcliff_obershelp(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(ratcliff_obershelp=True)
            .popitem()[1][0]
            .sim(string_a, string_b)
        )
        == expected
...
def test_ncd_bz2n(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(ncd_bz2=True).popitem()[1][0].sim(string_a, string_b)
        )
        == expected
    )
...
def test_fuzzy_wuzzy_partial_string(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(fuzzy_wuzzy_partial_string=True)
            .popitem()[1][0]
            .sim(string_a, string_b)
        )
        == expected
...
def test_fuzzy_wuzzy_token_sort(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(fuzzy_wuzzy_token_sort=True)
            .popitem()[1][0]
            .sim(string_a, string_b)
        )
        == expected
...
def test_fuzzy_wuzzy_token_set(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(fuzzy_wuzzy_token_set=True)
            .popitem()[1][0]
            .sim(string_a, string_b)
        )
        == expected
...
def test_editexn(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(editex=True).popitem()[1][0].sim(string_a, string_b)
        )
        == expected
    )
...
def test_typo(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(typo=True).popitem()[1][0].sim(string_a, string_b)
        )
        == expected
    )
...
def test_lig_3(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(lig_3=True).popitem()[1][0].sim(string_a, string_b)
        )
        == expected
    )
...
def test_ssk(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(ssk=True).popitem()[1][0].sim(string_a, string_b)
        )
        == expected
    )
...
def test_refined_soundex(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(refined_soundex=True)
            .popitem()[1][0]
            .sim(string_a, string_b)
        )
        == expected
...
def test_double_metaphone(string_a, string_b, expected):
    assert (
        pytest.approx(
            make_distance_metrics(double_metaphone=True)
            .popitem()[1][0]
            .sim(string_a, string_b)
        )
        == expected
