...
def match_names(data_first: Union[pd.DataFrame, pd.Series],
    ...
    **kwargs
    ...
    def check_data(data: Union[pd.Series, pd.DataFrame],
                   column: str,
    ...
    def preprocess_data(column: str,
                        data_first: pd.DataFrame,
                        data_second: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
        ...
        if not punctuation_sensitive:
            data_first[column] = data_first[column].str.replace('[^\w\s]', '')
    ...
    def combine_data(data_first: pd.DataFrame, 
        ...
        matches = matches[['company_name', 'company_name_matched', 'score', 'index']].rename(
            columns={'company_name': 'original_name', 'company_name_matched': 'match_name', 'index': 'match_index'})

        return matches

    def match_single(matcher: NameMatcher,
                     data_first: pd.DataFrame,
                     data_second: pd.DataFrame,
        ...
        matches = combine_data(data_first, data_second,
    ...
    def match_group(matcher: NameMatcher,
                    data_first: pd.DataFrame,
                    data_second: pd.DataFrame,
                    name_column: str,
                    group_column_first: str,
    ...
    data_first = check_data(data_first, column_first, group_column_first)
    data_second = check_data(data_second, column_second, group_column_second)
    ...
    data_first, data_second = preprocess_data(
        name_column, data_first, data_second)

    matcher = NameMatcher(**kwargs)

    if group_column_first == '':
        matches = match_single(matcher, data_first, data_second, name_column)
    else:
        matches = match_group(matcher, data_first, data_second,
